...
impl Connection {
    ...
    ///
    /// These are the client's addresses as they arrived in `REACH_OUT` frames, and they are
    /// what the current nat traversal round is probing. Only available on the server side of
    /// the connection; the client side has
    /// [`Connection::get_remote_nat_traversal_addresses`].
